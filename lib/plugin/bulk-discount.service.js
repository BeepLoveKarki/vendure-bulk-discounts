...
exports.BulkDiscountService = void 0;
...
let BulkDiscountService = class BulkDiscountService {
    ...
    findAll(ctx, options) {
        return this.connection.getRepository(ctx, bulk_discount_entity_1.BulkDiscount).find(options);
    }
    findByProductVariantSku(ctx, productVariantSku) {
        return this.connection
            .getRepository(ctx, bulk_discount_entity_1.BulkDiscount)
    ...
    findByProductVariantId(ctx, productVariantId) {
        return this.connection
            .getRepository(ctx, bulk_discount_entity_1.BulkDiscount)
            .createQueryBuilder("bulkDiscount")
            .leftJoinAndSelect("bulkDiscount.productVariant", "productVariant")
    ...
    async findProductVariantIdBySku(ctx, sku) {
        return core_1.assertFound(this.connection.getRepository(ctx, core_1.ProductVariant).findOne({ where: { sku } })).then((v) => {
            return v.id;
        });
    }
    ...
    async create(ctx, input) {
        ...
        for (const d of input.discounts) {
            ...
            discounts.push(core_1.assertFound(this.connection.getRepository(ctx, bulk_discount_entity_1.BulkDiscount).save(discount)));
        }
        return Promise.all(discounts);
    }
    async update(ctx, id, quantity, price) {
        return this.connection
            .getRepository(ctx, bulk_discount_entity_1.BulkDiscount)
    ...
    async delete(ctx, ids) {
        try {
            await this.connection.getRepository(ctx, bulk_discount_entity_1.BulkDiscount).delete(ids);
            return {
