...
exports.ProductVariantEntityResolver = exports.BulkDiscountEntityResolver = exports.BulkDiscountShopResolver = exports.BulkDiscountAdminResolver = void 0;
...
let BulkDiscountAdminResolver = class BulkDiscountAdminResolver {
    ...
    async updateProductVariantBulkDiscounts(ctx, args) {
        ...
        if (discounts.length < args.discounts.length) {
            ...
            this.bulkDiscountService.delete(ctx, discounts
                .slice(args.discounts.length, discounts.length)
                .map((d) => d.id));
        }
        const updates = [];
        for (let i = 0; i < discounts.length; i++) {
            updates.push(this.bulkDiscountService.update(ctx, discounts[i].id, args.discounts[i].quantity, args.discounts[i].price));
        }
        await Promise.all(updates);
        return true;
    }
    async updateProductVariantBulkDiscountsBySku(ctx, args) {
        const productVariantId = await this.bulkDiscountService.findProductVariantIdBySku(ctx, args.productVariantSku);
        let discounts = await this.bulkDiscountService.findAll(ctx, {
            where: { productVariant: productVariantId },
        });
        if (discounts.length < args.discounts.length) {
            discounts = discounts.concat(await this.bulkDiscountService.create(ctx, {
                productVariantId: productVariantId,
                discounts: args.discounts.slice(discounts.length, args.discounts.length),
            }));
        }
        else if (args.discounts.length < discounts.length) {
            await this.bulkDiscountService.delete(ctx, discounts
                .slice(args.discounts.length, discounts.length)
                .map((d) => d.id));
...
let ProductVariantEntityResolver = class ProductVariantEntityResolver {
    ...
    async bulkDiscounts(ctx, productVariant) {
        return this.bulkDiscountService.findByProductVariantId(ctx, productVariant.id);
    }
};
__decorate([
    graphql_1.ResolveField(),
