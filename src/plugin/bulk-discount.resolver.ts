...
export class BulkDiscountAdminResolver {
  ...
  async updateProductVariantBulkDiscounts(
    ...
    if (discounts.length < args.discounts.length) {
      ...
      this.bulkDiscountService.delete(ctx,
        discounts
    ...
    for (let i = 0; i < discounts.length; i++) {
      updates.push(
        this.bulkDiscountService.update(ctx,
          discounts[i].id,
  ...
  async updateProductVariantBulkDiscountsBySku(
    ...
    const productVariantId = await this.bulkDiscountService.findProductVariantIdBySku(ctx,
    ...
    if (discounts.length < args.discounts.length) {
      discounts = discounts.concat(
        await this.bulkDiscountService.create(ctx,{
      ...
      await this.bulkDiscountService.delete(ctx,
        discounts
          .slice(args.discounts.length, discounts.length)
          .map((d) => d.id)
      );
